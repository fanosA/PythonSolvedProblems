...
The problem, of course, is that you don’t know the shift. But our encryption method only has 26 distinct possible values for the shift! We know English is the main language of these emails,
so if we can write a program that tries each shift and maximizes the number of English words in the decoded message, we can decrypt their cipher!
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(26):
            wordCount = 0
            decryptedStr = self.apply_shift(s)
            strList = decryptedStr.split(' ')
