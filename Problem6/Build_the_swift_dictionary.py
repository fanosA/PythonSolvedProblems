...
ABCDEFGHIJKLMNOPQRSTUVWXYZ
A reminder from the introduction page - characters such as the space character, commas, periods, exclamation points, etc will not be encrypted by this cipher - basically,
all the characters within string.punctuation, plus the space (' ') and all numerical characters (0 - 9) found in string.digits.
...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(string.ascii_lowercase)):
            self.dict[string.ascii_lowercase[i]]=string.ascii_lowercase[(i+shift)%len(string.ascii_lowercase)]
            self.dict[string.ascii_uppercase[i]]=string.ascii_uppercase[(i+shift)%len(string.ascii_uppercase)]
        return self.dict
